...
def parse_string_into_merge_yolo_action(results: str) -> dict[int, UpdateResponse]:
    ...
    for l in lines:
        ...
        parse_line = [t.strip() for t in clean_line.split(CONFIG.llm_tab_separator)]
        parse_line = [t for t in parse_line if t]
        if len(parse_line) < 1:
            continue
        if parse_line[0].upper() == "ACTION" and len(parse_line) >= 2:
            action = parse_line[1].upper()
            memo_parts = parse_line[2:]
        else:
            action = parse_line[0].upper()
            memo_parts = parse_line[1:]
        if action not in MERGE_ACTION_SPACE:
            continue
        if memo_parts:
            memo = CONFIG.llm_tab_separator.join(memo_parts).strip()
        else:
            memo = action
